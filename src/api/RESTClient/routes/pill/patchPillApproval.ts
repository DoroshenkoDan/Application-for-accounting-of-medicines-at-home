 ...
 * @example
 * try {
 *   const { data } = await client.api.Pills.patchApproved({ Pill_id: 1 }, { is_approved: true });
 *   console.log(data);
 * } catch (error) {
 *   console.error(error);
 * }
...
export interface TAPIPatchPillApproval {
...
export type TError = TAPIPatchPillApproval['TError'];
export type TSuccess = TAPIPatchPillApproval['TSuccess'];
export type TParams = TAPIPatchPillApproval['TParams'];
export type TPayload = TAPIPatchPillApproval['TPayload'];

export async function patchPillApproval(
  this: BaseRESTClient,
  { Pill_id }: TParams,
  payload: TPayload,
